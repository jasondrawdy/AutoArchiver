...
namespace ICSharpCode.SharpZipLib.Core
{
	...
	{
        private static bool Throw = false;
		...
		/// <param name="destination">The stream to write data to.</param>
		/// <param name="buffer">The buffer to use during copying.</param>
		static public async Task<bool> CopyAsync(Stream source, Stream destination, byte[] buffer)
        {
            if (source == null)
            {
                return false;
            ...
            while (copying)
            {
                ...
                if (bytesRead > 0)
                {
                    if (Throw) throw new Exception("User requested cancellation");
        ...
        public static void ThrowException() { Throw = true; }
        public static void ResetFlags() { Throw = false; }
	}
}
